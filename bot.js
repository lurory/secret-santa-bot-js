  ...
  switch (command) {
    ...
    case '/foto':
      ...
      bot.getUserProfilePhotos(user.id, { 'limit': 1 }).then(function (result) {
        ...
        bot.getFile(file_id).then(function (file) {
    ...
    case '/criar':
      ...
      if (createResult === true)
        bot.sendMessage(chatId, 'Novo amigo oculto criado por ' + user.first_name + '.')
      else
        bot.sendMessage(chatId, 'Já existe um amigo oculto para esse grupo. Digite /entrar para entrar no amigo oculto.',
          {
    ...
    case '/entrar':
      ...
      if (fields.length < 2)
        ...
        person = new Person(user, fields.slice(1, fields.length).join(' '))
      ...
      if (addResult === true && fields.length >= 2)
        ...
        bot.sendMessage(chatId, 'Agora você está participando do amigo oculto. Quando decidir seu presente, use o comando /presente.\nEx: /presente barra de chocolate',
          {
            'reply_to_message_id': msg.message_id,
          })
      else
  ...
  const results = [
    {
      ...
      input_message_content: { message_text: 'Beltrano viu quem ele tirou!' },
